...
struct usb_device 
{
	...
	int maxchild;			/* Number of ports if hub 如果是个hub，该hub拥有的插口总数*/
	struct usb_device *parent;
	struct usb_device *children[USB_MAXCHILDREN];//包含的子设备 hub在探测子设备时，默认将几号插口插的设备固定在index的位置
	void *hc_private;//root ohci
	void *match;
