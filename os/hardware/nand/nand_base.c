	...
	while(1) 
	{
		...
		if (realpage != chip->pagebuf || oob) 
		{
			...
			if(DataBuf!=NULL)
			{
				for(i=0;i<2048;i++)
				{
					DataBuf[i]=((struct ls1x_nand_info*)(mtd->priv))->data_buff[i];
				}
				showhex(DataBuf,2048);
			}
			while(1)
				;
			*/
		     ...
		     size_t *retlen, uint8_t *buf)
{
	...
	DEBUG_YA5(ret);DEBUG_YA5(chip->ops.retlen);
...
static int nand_block_isbad(struct mtd_info *mtd, loff_t offs)
{
	...
	if (offs > mtd->size)
		return -EINVAL;
	int xx=nand_block_checkbad(mtd, offs, 1, 0);
	DEBUG_YA5(xx);
	return  xx;
}
						  ...
						  int busw, int *maf_id)
{
	...
	*maf_id = chip->read_byte(mtd);
	...
	rt_kprintf(KERN_INFO "xNAND device: Manufacturer ID:"
	       " 0x%02x, Chip ID: 0x%02x (%s %s)\n", *maf_id, dev_id,
...
int nand_scan_ident(struct mtd_info *mtd, int maxchips)
{
	...
	if (i > 1)
		rt_kprintf("%d NAND chips detected\n", i);
