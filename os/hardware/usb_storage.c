...
static int usb_storage_dev_index = 0; /* number of highest available usb device */
...
void storage_free(struct usb_device *dev)//dstling
{
/*
...
int usb_storage_probe(struct usb_device *dev, unsigned int ifnum,struct us_data *ss)
{
	...
	//printf("USB Mass Storage device detected\n");
...
int	usb_read(dev_t dev, struct uio *uio, int ioflag)
{
	...
	//int ret= physio(usb_strategy, NULL, dev, B_READ, NULL, uio);
	return 1;//ret;
}
...
int usb_read_dst(int fd,void *buf,size_t blen)
{
	unsigned int blkno, blkcnt;
	unsigned int d_secsize; 
	struct  block_dev_desc_t *block_usb;
	int ret ;
	int usb_dev_index=opendevs[fd].dev_index;
	
	blkno = btodb(opendevs[fd].offs);
	d_secsize = usb_dev_desc[usb_dev_index].blksz;
	...
	blkcnt = howmany(blen, d_secsize); 
	
	if(B_READ)//这里先不管flag了
	{
		if((unsigned long)buf & 0x7f)
		{
			//printf("usb_strategy1,dev:%d, blkno:%d, blkcnt:%d,bulkbuf:0x%08x, buf:0x%08x\n",dev, blkno, blkcnt, (unsigned long *)bulkbuf,buf);
			ret = usb_stor_read(usb_dev_index, blkno, blkcnt, (unsigned long *)bulkbuf, buf);
		} 
		else 
		{
			//printf("usb_strategy2,dev:%d, blkno:%d, blkcnt:%d,buf:0x%08x\n",dev, blkno, blkcnt, buf);
			ret = usb_stor_read(usb_dev_index, blkno, blkcnt, (unsigned long *)buf, NULL);
		}
		if(ret != blkcnt)
			printf("usb_read_dst error\n");	
		//dotik(30000, 0);
	}
}

void add_usb_storage_device(struct usb_device *dev)
//{
//}
//int usb_storage_match(struct device *parent, void *match, struct usb_device *dev)
{
	pci_sync_cache(0, _usb_ccb, sizeof(_usb_ccb), 1);
	usb_ccb = (ccb*)CACHED_TO_UNCACHED(&_usb_ccb);
	if(usb_storage_dev_index==USB_MAX_STOR_DEV) 
	{
		printf("max USB Storage Device reached: %d stopping\n",usb_storage_dev_index);
		return -1;	
	}

	if(usb_storage_probe(dev,0,&usb_stor[usb_storage_dev_index])) 
	{ // ok, storage devices 
		memset(&usb_dev_desc[usb_storage_dev_index],0,sizeof(block_dev_desc_t));
		usb_dev_desc[usb_storage_dev_index].target=0xff;
		usb_dev_desc[usb_storage_dev_index].if_type=IF_TYPE_USB;
		usb_dev_desc[usb_storage_dev_index].dev= usb_storage_dev_index;
		usb_dev_desc[usb_storage_dev_index].part_type=PART_TYPE_UNKNOWN;
		usb_dev_desc[usb_storage_dev_index].block_read=&usb_stor_read;
		// get info and fill it in 
		if(usb_stor_get_info(dev, &usb_stor[usb_storage_dev_index], &usb_dev_desc[usb_storage_dev_index]))
		{
			struct usb_found_dev *new_usb_dev=(struct usb_found_dev*)malloc(sizeof(struct usb_found_dev));
			memset(new_usb_dev,0,sizeof(struct usb_found_dev));
			sprintf(new_usb_dev->name,"usb%d",usb_storage_dev_index);
			new_usb_dev->index=usb_storage_dev_index;
			new_usb_dev->dev=dev;
			//添加至usb设备链表
			usb_new_device_insert_list(new_usb_dev);
			
			printf("USB Mass Storage device detected:%s\n",new_usb_dev->name);
			usb_storage_dev_index++;
			return 1;
		}
	} // if storage device 
...
//啥事没做
void usb_storage_attach(struct device *parent, struct device *self, void *aux)
{
	struct usb_device * dev = aux;
	//dev->match = self;
	//dev->destruct = storage_free;
	usb_storage_notify(dev);//noting to do
}
