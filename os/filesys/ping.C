...
static int com_ping(int argc, char **argv)
{
	...
	while ((ch = getopt(argc, argv, "Rc:dfg:h:i:l:p:qrs:v")) != EOF)
		...
		case 'c':
			...
			if (npackets <= 0) 
			{
				(void)printf("ping: bad number of packets to transmit.\n");
				exit(EXIT_FAILURE);
			}
			break;
		case 'd':
		...
		case 'g':
			...
			if (groupsize < 0) {
				(void)printf("ping: bad groupsize value.\n");
				exit(1);
			}
			options |= F_GROUP;
			break;
		case 'i':		/* wait between sending packets */
			...
			{
				(void)printf("ping: bad timing interval.\n");
				exit(1);
			}
			options |= F_INTERVAL;
			break;
		case 'l':
			preload = atoi(optarg);
			if (preload < 0) {
				(void)printf( "ping: bad preload value.\n");
				exit(EXIT_FAILURE);
			}
			break;
		case 'q':
		...
		case 's':		/* size of packet to send */
			...
			if (datalen > MAXPACKET) {
				(void)printf("ping: packet size too large.\n");
				exit(EXIT_FAILURE);
			}
			if (datalen <= 0) {
				(void)printf("ping: illegal packet size.\n");
				exit(EXIT_FAILURE);
			}
			break;
		case 'v':
	...
	else 
	{
		...
		if (!hp) 
		{
			(void)printf("ping: unknown host %s\n", target);
			exit(EXIT_FAILURE);
		}
	...
	{
		(void)printf("ping: -f and -i/-g incompatible options.\n");
		exit(EXIT_FAILURE);
	}
	...
	{
		(void)printf("ping: unknown protocol icmp.\n");
		exit(EXIT_FAILURE);
	}
...
#else
		(void)printf("ping: record route not available in this implementation.\n");
		exit(EXIT_FAILURE);
#endif /* IP_OPTIONS */
	}
	...
	if (to->sin_family == AF_INET)//yes
		(void)printf("PING %s (%s): %d data bytes\n", hostname,inet_ntoa(*(struct in_addr *)&to->sin_addr.s_addr),datalen);
	else
	...
	for (;;) 
	{
		...
		if (options & F_FLOOD) //no??
		{
			pinger();
			timeout.tv_sec = 0;
			timeout.tv_usec = 10000;
			fdmask = 1 << s;//sys_select
			DebugPR;
			if (select(s + 1, (fd_set *)&fdmask, (fd_set *)NULL,(fd_set *)NULL, &timeout) < 1)
				continue;
		}
		//DebugPR;
		fromlen = sizeof(from);//sys_recvfrom
		if ((cc = recvfrom(s, (char *)packet, packlen, 0,(struct sockaddr *)&from, &fromlen)) < 0) 
		{
			if (errno == EINTR)
				continue;
			//perror("ping dst: recvfrom");//dstling
			//DebugPR;
			continue;
		}
		//DebugPR;
		pr_pack((char *)packet, cc, &from);//显示接收到的包状态
		//DebugPR;
...
static void catcher(void)
{
	...
	{
		(void)signal(SIGALRM, (sig_t)catcher);
		setitimer(ITIMER_REAL, &interval, (struct itimerval *)NULL);//sys_setitimer
	} 
	else 
	{
		waittime = MAXWAIT;
...
static void pr_pack(char *buf,int cc, struct sockaddr_in *from)
{
	...
	if (icp->icmp_type == ICMP_ECHOREPLY) 
	{
		...
		else 
		{
			...
			if (timing)
				printf(" time=%d.%03d ms", triptime/1000,triptime % 1000);
		}
		...
		if (options & F_VERBOSE) 
		{
		    static int lastseq;
		    if (dupflag)
			printf(" (DUP! id=%u prev=%u)", icp->icmp_seq,lastseq);
